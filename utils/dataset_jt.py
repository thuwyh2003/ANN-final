...
class CUB(Dataset):# WYH
    def __init__(self, root, is_train=True, batch_size=1, shuffle=False,  transform=None):
        super(CUB,self).__init__(root,is_train, batch_size, False,transform)
        self.root = root
...
class CarsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        car_class = jt.array(np.array(car_class).astype(np.float32))
        car_class = car_class.long()
...
def download_url(url, root, filename):
    """
    Download a file from a URL to a local directory, with optional MD5 checksum verification.

    Args:
        url (str): URL to download the file from.
        root (str): Directory to place downloaded file.
        filename (str): Name of the file to save as.
        md5 (str, optional): MD5 checksum of the download for verification.
...
class dogs(Dataset):
    ...
    def download(self):
        ...
        for filename in ['images', 'annotation', 'lists']:
            ...
            download_url(url, self.root, tar_filename)
...
class NABirds(Dataset):
    ...
    def __init__(self, root, shuffle=False,batch_size=1,train=True, transform=None):
...
# modify most of the code to fit jittor
def verify_str_arg(value, arg=None, valid_values=None, custom_msg=None):
    """
    Verify that the input is a string and optionally check that it is one of the
    expected values.

    Args:
        value (str): The value to be checked.
        arg (str, optional): The name of the argument to be checked. This is used
            for error messages.
        valid_values (iterable of str, optional): An iterable of valid values.
            The default is None, which means that any string is allowed.
        custom_msg (str, optional): A custom error message to be used when raising
            a ValueError. If not provided, a default message is used.

    Raises:
        TypeError: If the input is not a string.
        ValueError: If the input is not one of the valid_values, when provided.
    """
    if not isinstance(value, str):
        raise TypeError(f"{arg} needs to be a string, but got {type(value)}")
    ...
    if valid_values is not None and value not in valid_values:
        if custom_msg is not None:
            ...
            raise ValueError(f"{arg} needs to be one of {valid_values_str}, but got {value}")

def extract_archive(from_path: str, to_path: Optional[str] = None):
    """
    Extracts a zip or tar archive to the specified directory.

    Args:
        from_path (str): The path to the archive file.
        to_path (str, optional): The path to the directory where the contents should be extracted.
            If None, the current directory is used.
        remove_finished (bool, optional): If True, the archive file is removed after extraction.
    ...
    elif from_path.endswith(('.tar.gz', '.tgz', '.tar.bz2', '.tbz', '.tar')):
        mode = 'r:gz' if from_path.endswith('.gz') else 'r:bz2' if from_path.endswith('.bz2') else 'r'
        with tarfile.open(from_path, mode) as archive:
            archive.extractall(to_path)
    else:
        raise ValueError(f'Unsupported archive type: {from_path}')

def check_integrity(file_path: str, expected_hash: str) -> bool:
    """
    Check the integrity of a file by comparing its SHA256 hash with an expected hash.

    Args:
        file_path (str): The path to the file to check.
        expected_hash (str): The expected SHA256 hash value of the file.
    ...
    # Compare the file's hash with the expected hash
